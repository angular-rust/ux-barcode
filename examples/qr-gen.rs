...
fn main() {
    ...
    loop {
        match iter.next() {
            Some(v) if v == "-o" => {
                output = Some(iter.next().expect("missing output path").to_string())
            }
    ...
    let _ = args
        ...
        .skip(1)
        .next()
        .map(|text| generate_qrcode(text, &outpath));
}
...
fn generate_qrcode(text: &str, outpath: &str) {
    ...
    pixmap
        .save_png(outpath)
        .expect("failed to generate PNG file");

    // open if on macos
    if env::consts::OS.to_lowercase() == "macos" {
        let _ = Command::new("/usr/bin/open").arg(outpath).output();
    } else {
