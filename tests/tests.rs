...
use barcode::symbols::qr::QrCodeEcc;

const FOLDER: &str = concat_with_file_separator!("tests", "data");

#[test]
fn text_to_matrix() {
    let result = barcode::to_matrix("Hello world!", QrCodeEcc::Low).unwrap();
...
fn text_to_svg_to_string() {
    let result = barcode::to_svg_to_string("Hello world!", QrCodeEcc::Low, 256, Some("")).unwrap();

    assert_eq!(
        fs::read_to_string(Path::new(FOLDER).join("hello.svg")).unwrap(),
        result
    );
}
...
fn text_to_png_to_vec() {
    let result = barcode::to_png_to_vec("Hello world!", QrCodeEcc::Low, 256).unwrap();
